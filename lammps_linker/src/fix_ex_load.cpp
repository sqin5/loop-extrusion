...
FixExLoad::FixExLoad(LAMMPS *lmp, int narg, char **arg) :
  ...
  probability(NULL), created(NULL), copy(NULL), random(NULL), list(NULL),ccount(NULL)
{
...
void FixExLoad::setup(int /*vflag*/)
{
...
void FixExLoad::post_integrate()
{
  ...
  if (atom->nmax > nmax) {
    ...
    memory->create(ccount,nmax,"ex_load:ccount");
    probability = distsq;
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
     ...
     // if (abs(tag[i]-tag[j]) == 2)
      ...
      // new added loop extrusion ,need to be i i+2
      if (abs(tag[i]-tag[j]) != load_gap) 
      continue;
      
      if (ccount[i] == 1 || ccount[j] ==1)
      continue;  
     ...
     // }
      ...
      if (num_bond[i] != 1) continue;
      if (num_bond[j] != 1) continue;
      if (bondcount[i] != 0) continue;
      if (bondcount[j] != 0) continue; 
      if (molecule[i]!=molecule[j]) continue;

      
        if (tag[i] < tag[j]) {
        for (k = 1; k < load_gap; k++) {
        int mid_tag = tag[i] + k;
        int mid_atom = atom->map(mid_tag);
        if ( num_bond[mid_atom] != 1 || partner[mid_atom] != 0) {
           valid =false;
           break;
          }
        }
      ...
      //if (abs(tag[i]-tag[j]) == load_gap && molecule[i]==molecule[j])

      
       
      //this is only for load_gap = 2 
     // if (num_bond[atom->map((tag[i] + tag[j]) / 2)] != 1) continue;
...
*/
    ...
    fprintf(logfile,"loading:\n %ld, %d, %d, %d, %d\n", update->ntimestep, tag[i], bondcount[i],tag[j], bondcount[j]);
