...
PairLJCut::~PairLJCut()
{
  if (allocated) {
    ...
    memory->destroy(numA_list);
    memory->destroy(numB_list);
    memory->destroy(cut);
...
void PairLJCut::compute(int eflag, int vflag)
{
  ...
  repeat_flag=0;
  check_dist=3.0*3.0;
  memset(&(numA_list[0]), 0, 12000*sizeof(int));
  memset(&(numB_list[0]), 0, 12000*sizeof(int));
  for (ii = 0; ii < inum; ii++) {
    i = ilist[ii];
    xtmp = x[i][0];
    ytmp = x[i][1];
...
/*
if (me==0) {
  memory->create(numA_receive_list,nprocs*12000,"pair:numA_receive_list");
  memory->create(numB_receive_list,nprocs*12000,"pair:numB_receive_list");
  memset(&(numA_receive_list[0]), 0, nprocs*12000*sizeof(int));
  memset(&(numB_receive_list[0]), 0, nprocs*12000*sizeof(int));
  }
   MPI_Barrier(world);

   MPI_Gather(numA_list,12000,MPI_INT,numA_receive_list,12000,MPI_INT,0,world);
   MPI_Gather(numB_list,12000,MPI_INT,numB_receive_list,12000,MPI_INT,0,world);
   MPI_Reduce(&count,&count_total,1,MPI_INT,MPI_SUM,0,world);
   
   if (me==0 && step%freq==0) {
   fprintf(logfile,"%d\n",count_total);
   for (ii = 0; ii < nprocs*12000; ii++) {
   if (numA_receive_list[ii]!=0) {
   if (numA_receive_list[ii] < numB_receive_list[ii])
   fprintf(logfile,"%d %d %d \n",step,numA_receive_list[ii],numB_receive_list[ii]);
   else
   fprintf(logfile,"%d %d %d \n",step,numB_receive_list[ii],numA_receive_list[ii]); 
   }
   }
   }
*/
 ...
 // if (me==0) {
 // memory->destroy(numA_receive_list);
 // memory->destroy(numB_receive_list);
 // }
}
...
void PairLJCut::allocate()
{
  ...
  memory->create(numA_list,12000,"pair:numA_list");
  memory->create(numB_list,12000,"pair:numB_list");
