  ...
  broken(NULL), copy(NULL), random(NULL)
{
  ...
  if (narg > 9) {
  // printf("jinqule= \n %d %d\n",iarg,narg);
  inewtype = force->inumeric(FLERR,arg[iarg]); }
  printf("jinqule= \n %d %d %d\n",iarg,narg,inewtype);
  // error chec 
...
void FixExUnload::post_integrate()
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    //printf("jinqule= \n %d %d %d %d\n",i,tag[i],atom->type[i],inewtype);
//	 atom->type[i] = inewtype;
  ...
  // error chec 
    ...
    fprintf(logfile,"unloading:\n %ld, %d, %d, %d, %d\n", update->ntimestep, tag[i], atom->type[i],tag[j], atom->type[j]);
