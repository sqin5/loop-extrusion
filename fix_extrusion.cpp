                                                                ...
                                                                random(NULL), bondcount(NULL), recreate(NULL)
{
  ...
  if (narg > 9)
  {
    ...
    safe_distance = force->numeric(FLERR, arg[12]);
    ioldtype = force->inumeric(FLERR, arg[13]);
    inewtype = force->inumeric(FLERR, arg[14]);
  }
  else
  {
    ctcf_left_right = -1;
...
void FixExtrusion::post_integrate()
{
  ...
  bool l_flag;
  int i_old, j_old, i_new, j_new; //adjust type
...
//printf ("bonds of 3:\n %d, %d, %d, %d, %d, %d\n", tag[i1], num_bond[i1],bondcount[i1], tag[i2], num_bond[i2],bondcount[i2]);
    if (
        bondcount[i1] != 1 ||
        bondcount[i2] != 1)
...
/*   
      i_old = i1;
      j_old = i2;
      i_new = atom->map(tag[i1] - 1);
      j_new = atom->map(tag[i2] + 1);
  */   
    fprintf(logfile,"extrusion_start:\n %ld, %d, %d, %d, %d, %d, %d\n", update->ntimestep, tag[atom->map(tag[i1] )], num_bond[atom->map(tag[i1])],bondcount[atom->map(tag[i1])],tag[atom->map(tag[i2])], num_bond[atom->map(tag[i2])],bondcount[atom->map(tag[i2])]);
   
        fprintf(logfile,"extrusion_start_2:\n %ld, %d, %d\n", update->ntimestep, atom->type[atom->map(tag[i1])],atom->type[atom->map(tag[i2])] );
    ...
    if (
        ...
        atom->type[atom->map(tag[i1] - 1)] == linker) &&
        (atom->type[atom->map(tag[i1] - 1)] != ctcf_left ||
        through_prob > random->uniform()) &&
        (atom->type[atom->map(tag[i1] - 1)] != ctcf_left_right ||
        through_prob > random->uniform()) &&
        (atom->type[atom->map(tag[i1] - 1)] != linker ||
        linker_prob > random->uniform()) )
    {
    ...
    // printf("check 1=%d\n",local_left);
      if (
          ...
          atom->type[atom->map(tag[i2] + 1)] == linker) &&
          (atom->type[atom->map(tag[i2] + 1)] != ctcf_right ||
          through_prob > random->uniform()) &&
          (atom->type[atom->map(tag[i2] + 1)] != ctcf_left_right ||
          through_prob > random->uniform()) &&
      ...
      // printf("check 3=%g %g\n",rsq,safe_dsq);
        ...
        fprintf(logfile,"extrusion_left_right:\n %ld, %d, %d, %d, %d\n", update->ntimestep, tag[atom->map(tag[i1] - 1)], num_bond[atom->map(tag[i1] - 1)],tag[atom->map(tag[i2] + 1)], num_bond[atom->map(tag[i2] + 1)]);
      ...
      { // left move, right not
        ...
        fprintf(logfile,"extrusion_left_only:\n %ld, %d, %d, %d, %d\n", update->ntimestep, tag[atom->map(tag[i1] - 1)], num_bond[atom->map(tag[i1] - 1)],tag[atom->map(tag[i2])], num_bond[atom->map(tag[i2])]);
      }
    }
    else if (
        bondcount[atom->map(tag[i2] + 1)] == 0 &&
...
//  printf("check 4=%g %g\n",rsq,safe_dsq);  
      ...
      fprintf(logfile,"extrusion_right_only:\n %ld, %d, %d, %d, %d\n", update->ntimestep, tag[atom->map(tag[i1])], num_bond[atom->map(tag[i1])],tag[atom->map(tag[i2]+1)], num_bond[atom->map(tag[i2]+1)]);
    }
  }
  bool iisleft = false;
  for (i = 0; i < nlocal; i++)
  ...
  for (i = 0; i < nlocal; i++)
  {
        ...
        (to_add[atom->map(lb)] == rb + 1 && to_add[atom->map(rb + 1)] == lb))
    {
      i_old = atom->map(lb);
      j_old = atom->map(rb);
      if (to_add[atom->map(lb - 1)] == rb && to_add[atom->map(rb)] == lb - 1) {
        i_new = atom->map(lb-1);
        j_new = atom->map(rb); }
      if (to_add[atom->map(lb - 1)] == rb+1 && to_add[atom->map(rb+1)] == lb - 1) {\
        i_new = atom->map(lb-1);
        j_new = atom->map(rb+1); }
      if (to_add[atom->map(lb)] == rb+1 && to_add[atom->map(rb+1)] == lb) {\
        i_new = atom->map(lb);
        j_new = atom->map(rb+1); }
...
/*         
      atom->type[i_old] = ioldtype;
      atom->type[j_old] = ioldtype;
      atom->type[i_new] = inewtype;
      atom->type[j_new] = inewtype;
*/
